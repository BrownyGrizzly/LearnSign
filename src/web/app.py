...
def predict():
    data = request.json
    chosen_word = data.get('word', None)
    sequence = []
    try:
        with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
        ...
        app.logger.error(f"UnicodeEncodeError: {str(e)}")
        return jsonify({'status': 'failed', 'message': 'Unicode encoding error occurred'})
    except Exception as e:
        app.logger.error(f"Exception: {str(e)}")
        return jsonify({'status': 'failed', 'message': 'An error occurred during prediction'})
